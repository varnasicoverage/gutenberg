...
/**
 * Extracts Heading (`core/heading`) blocks from a post.
 *
 * @param WP_Post $post The post to extract Heading blocks from.
 *
 * @return array The list of Heading blocks.
 */
function block_core_table_of_contents_get_heading_blocks( $post ) {
...
function block_core_table_of_contents_render_list( $nested_heading_list ) {
	$child_nodes = array_map(
		...
		$nested_heading_list, array_keys( $nested_heading_list )
	);
...
function render_block_core_table_of_contents( $attributes ) {
	...
	$heading_blocks = block_core_table_of_contents_get_heading_blocks( $post );
